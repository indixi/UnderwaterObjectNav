...
The existing `duo_gfl_project` remains the detector training/inference project.
...
Run from `UnderwaterObjectNav/src/object_Nav` with the workspace on `PYTHONPATH`:
...
To use the DUO detector, construct `MMDetSemanticDetector` in a short wrapper
and pass it as `detector` to `preprocess_dataset.build`. Cached detector JSON
from `duo_gfl_project/tools/infer.py` can be consumed by `JsonDetectionDetector`.
