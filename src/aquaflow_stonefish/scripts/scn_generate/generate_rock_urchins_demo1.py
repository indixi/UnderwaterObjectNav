...
def generate(package_dir: Path, seed: int, on_rock_count: int, beside_count: int):
    ...
    occupied = []
    output = [BEGIN, f'\t<!-- seed={seed}; on-rock={on_rock_count + 2} models (including 2 purple clusters); beside={beside_count}. -->']

    # Two large cluster assets are deliberately restricted to the two broadest rocks.
    for index, rock in enumerate((ROCKS[2], ROCKS[5]), 1):
        kind = "purple"
...
def main():
    ...
    parser.add_argument("--on-rock", type=int, default=18, help="single-urchin models on rocks; two purple clusters are added separately")
    parser.add_argument("--beside", type=int, default=4)
    args = parser.parse_args()
    if args.on_rock < 0 or args.beside < 0:
        parser.error("counts must be non-negative")
    generated = generate(package_dir, args.seed, args.on_rock, args.beside)
    update_scenario(args.scenario.resolve(), generated)
    print(f"Updated {args.scenario} with seed {args.seed}: {args.on_rock + 2} on-rock models, {args.beside} beside-rock models")
